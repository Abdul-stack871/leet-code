// Last updated: 7/14/2026, 2:22:59 PM
...
class Solution {
    public int[] pathExistenceQueries(int n, int[] nums, int maxDiff, int[][] queries) {
        ...
        Arrays.sort(valueWithIdx, (a, b) -> Integer.compare(a[0], b[0]));
        ...
        for (int i = 0; i < n; i++) {
            if (reach < i) reach = i;
            while (reach + 1 < n && 
                   valueWithIdx[reach + 1][0] - valueWithIdx[reach][0] <= maxDiff && 
        ...
        for (int i = 0; i < queries.length; i++) {
            int leftPos = sortedPos[queries[i][0]];
            int rightPos = sortedPos[queries[i][1]];
